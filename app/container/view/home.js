...
import {
    View,StyleSheet,Image,Text,TouchableHighlight,ScrollView,ListView,StatusBar,Animated,NavigatorIOS,RefreshControl,AlertIOS
} from 'react-native';
import Util from './../../core/utils';
import Tmall from './sub/tmall';
//菜单列表
...
export default class extends Component {
    ...
    _onRefresh() {
        ...
        fetch('https://api.github.com/search/repositories?q=javascript&sort=stars').then((res)=>res.text()).then((res)=> {
            ...
            //_this.setState({datasource: data.items});
        ...
        return (
            <View style={styles.container}>
                <ScrollView style={styles.con}
                    ...
                    {
                        this.state.datasource.map((e, i)=>function (e, i) {
                            return (<Text key={i}>{e.id}==></Text>);
                        })
                    }
...
const styles = StyleSheet.create({
    ...
    con: {width: Util.size.width, height: Util.size.height - 60},
    icon: {
        width: 58, height: 58
    },
    img: {
    ...
    Dlist: {flexDirection: 'row', padding: 10, backgroundColor: '#ccc'},
    Lcon: {
        width: 29, height: 29
    },
    Rcon: {

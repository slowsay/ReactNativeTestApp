...
import {
    StyleSheet,
    Text,Image,
    View,TabBarIOS,ListView,PushNotificationIOS,TextInput,TouchableOpacity
} from 'react-native';
...
export  default class extends Component {
    ...
    render() {
        Util.nslog('----------tarbar----------');
        const {navigator,state,actions}=this.props;
        return (
            <TabBarIOS tintColor="#fe5100" barTintColor="#c6d3db">
                <TabBarIOS.Item icon={{uri: 'm0.png',scale:5}} selectedIcon={{uri:'m0s.png',scale:5}}
                                title="首页"
                                onPress={()=>this.changeTab('首页')}
                                badge={this.state.homenotifCount>0?this.state.homenotifCount:undefined}
                                selected={this.state.selectedTab==='首页'}>
                    <View style={styles.container}>
                        <Searchtool showmsg={()=>this._showMsg()} navigator={navigator}/>
                        <Homeview navigator={navigator} topflag={this.state.topflag} state={state} actions={actions}/>
                        <TouchableOpacity onPress={()=>this._TopOnPress()} style={styles.topng}>
                            <Image style={styles.icon} source={{uri:'top.png'}}/>
                        </TouchableOpacity>
                    </View>
                </TabBarIOS.Item>
                <TabBarIOS.Item icon={{uri: 'm1.png',scale:5}} selectedIcon={{uri:'m1s.png',scale:5}}
                                title="微淘"
